...
if [[ "${SEED_ENTRYPOINT_AS_PYRRHA:-0}" != "1" && $(id -u) -eq 0 ]]; then
  ...
  exec su -s /bin/bash -c "SEED_ENTRYPOINT_AS_PYRRHA=1 DATA_DIR=$DATA_DIR WALLET_FILE=$WALLET_FILE ADDRESS_FILE=$ADDRESS_FILE LOG_FILE=$LOG_FILE /usr/local/bin/seed-entrypoint.sh" pyrrha
fi
...
if [ ! -s "$ADDRESS_FILE" ]; then
  if [ ! -f "$WALLET_FILE" ]; then
    ...
    echo "[seed-entrypoint] Wallet already exists; deriving miner address..."
    if ! pyrrha-wallet-cli \
      --wallet-file "$WALLET_FILE" \
      --password "" \
      --command address >"$LOG_FILE" 2>&1; then
      echo "[seed-entrypoint] Failed to read existing wallet; contents of $LOG_FILE:" >&2
      cat "$LOG_FILE" >&2
      exit 1
    fi
  fi
  ...
  echo "[seed-entrypoint] Miner address recorded: $ADDRESS"
else
